...
function TrendingPhotoslibrary() {
  const [allImage, setAllImage] = useState([]);
  const [currentSlide, setCurrentSlide] = useState(0);

  useEffect(() => {
    fetch("http://localhost:3000/getimagelibrary")
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <><div className="trending_location">Library</div><div className="slideshow-container">
      {allImage.length === 0 ?
...
export default TrendingPhotoslibrary;
